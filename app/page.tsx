...
export default async function Home() {
  const { data: popular } = await supabase.from("populars").select();
  ...
  return (
    <>
      ...
      <div className="max-w-[1800px] w-[85%] mx-auto mt-5">
        ...
        <CarouselComponent array={popular} />
      </div>

      <div className="w-[85%] mx-auto  max-w-[1800px] mt-5">
        <h2 className="text-2xl tracking-widest mb-4">Shop By Categories</h2>

        <div className="grid grid-cols-2 lg:grid-cols-5 items-center gap-4 mb-2">
          {categories?.map((category: Category) => (
            <div key={category.id} className="relative w-full h-[300px] ">
              <Image
